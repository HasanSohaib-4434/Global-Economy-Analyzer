...
class MyGUI(QDialog):
    def __init__(self):
        ...
        # Connect the pushButton's click event to the corresponding functions
    ...
    def show_confidence(self):
        ...
        # Call the function to plot the histogram initially
      #  self.plot_hist()
...
class GraphDialog(QDialog):
    def __init__(self):
        ...
        #self.comboBoxg3.currentIndexChanged.connect(self.my)

        #self.comboBoxg4.currentIndexChanged.connect(self.my)
    ...
    def my(self):
        ...
        # Add specific items to the combo box
        print("here again")
        #self.comboBoxg.clear()
    ...
    def comboBoxg_changed(self):
        # Example function to execute when the combo box selection changes
    ...
    def scatter_plot(self):
        if self.country1 != "Countries":
            ...
            # Filter data for the selected countries and time period
            data_filtered = df[(df['Country'] == self.country1) & (df['Year'] >= start_year) & (df['Year'] <= end_year)]

            # Scatter plot with color gradient based on the timeline
            ...
            # Manually add colorbar to the left side and make it smaller
            ...
            cax = plt.gca().inset_axes([0.05, 0.1, 0.03, 0.8])  # Adjust position and size as needed
            cbar = plt.colorbar(sm, cax=cax, label='Year',
                                orientation='vertical')  # Use orientation='horizontal' for horizontal colorbar
            ...
            # Embed the Matplotlib plot into a PyQt5 widget
            canvas = FigureCanvas(plt.gcf())

            # Clear previous content from the vertical layout
            for i in reversed(range(self.verticalLayoutg.count())):
                self.verticalLayoutg.itemAt(i).widget().setParent(None)

            # Add the Matplotlib plot to your vertical layout
            self.verticalLayoutg.addWidget(canvas)

            # If you want to show the plot immediately, you can uncomment the following line
            # canvas.draw()
    ...
    def box_plot(self):
        if self.country1 != "Countries":
            ...
            # Box plot for Imports and Exports, grouped by decades
    ...
    def plot_histogram(self):
        ...
        if self.country1 != "Countries":
            ...
            # Group data by 10-year intervals and calculate the mean GDP for each interval
            ...
            # Set a grid with a solid line style
            plt.grid(True, linestyle='-', alpha=0.7)
    ...
    def f_country(self):
        self.comboBoxg.clear()
        self.comboBoxg2.clear()
        self.comboBoxg3.addItems(['Countries', ' Afghanistan ', ' Albania ', ' Algeria ', ' Andorra ', ' Angola ',
       ...
       ' Sint Maarten (Dutch part) ', ' South Sudan ', ' Sudan '])
        ...
        self.comboBoxg4.addItems(['Countries', ' Afghanistan ', ' Albania ', ' Algeria ', ' Andorra ', ' Angola ',
       ' Antigua and Barbuda ', ' Azerbaijan ', ' Argentina ',
       ' Australia ', ' Austria ', ' Bahamas ', ' Bahrain ',
       ' Bangladesh ', ' Armenia ', ' Barbados ', ' Belgium ',
       ' Bermuda ', ' Bhutan ', ' Bolivia (Plurinational State of) ',
...
class DescriptiveDialog(QDialog):
    ...
    def my(self):
        ...
        if self.country1 != 'Countries' :
            if selected_item == 'GDP' or selected_item =='ANI' or selected_item =='GNI' or selected_item =='Imports' or selected_item =='Exports':
                ...
                range_value = np.ptp(country_data)  # Peak-to-Peak (Range)
    ...
    def f_country(self):
        self.comboBoxd3.addItems(['Countries', ' Afghanistan ', ' Albania ', ' Algeria ', ' Andorra ', ' Angola ',
       ' Antigua and Barbuda ', ' Azerbaijan ', ' Argentina ',
       ' Australia ', ' Austria ', ' Bahamas ', ' Bahrain ',
       ' Bangladesh ', ' Armenia ', ' Barbados ', ' Belgium ',
       ' Bermuda ', ' Bhutan ', ' Bolivia (Plurinational State of) ',
...
class ProbabilityDialog(QDialog):
    ...
    def f_country(self):
        ...
        # Add code for your probability dialog initialization here
class RegressionDialog(QDialog):
    def __init__(self):
        super(RegressionDialog, self).__init__()
        loadUi("regression.ui", self)

        # Add code for your regression dialog initialization here
        self.f_country()
    ...
    def my(self):
        ...
        if self.country1 !='Countries':
            if selected_item == 'Population Prediction':
                ...
                # Make predictions for the year 2050 with a 20-year interval
                ...
                # Check if data is available for the specified country
                if country_data.empty:
                    raise ValueError(f"No data found for {self.country1}")

                # Extracting the 'GDP' and 'Population' columns
                gdp = country_data['GDP'].values.reshape(-1, 1)
                population = country_data['Population'].values.reshape(-1, 1)

                # Check if there is data in the extracted columns
                country_data = df[df['Country'] ==self.country1]  # Replace with the desired country name

                # Extracting the 'Year' and 'Population' columns
                years = country_data['Year'].values.reshape(-1, 1)
                population = country_data['Population'].values.reshape(-1, 1)
                ...
                # Store the predictions in three different variables
                ...
                # Example new population values for prediction
                ...
                # Make predictions for new population values
                ...
                # Extracting the 'GNI' and 'Exports' columns
                ...
                # Extracting the 'Year' and 'exports' columns
                years = country_data['Year'].values.reshape(-1, 1)
                population = country_data['Exports'].values.reshape(-1, 1)
    ...
    def f_country(self):
        self.comboBoxr2.addItems(['Countries', ' Afghanistan ', ' Albania ', ' Algeria ', ' Andorra ', ' Angola ',
                                  ' Antigua and Barbuda ', ' Azerbaijan ', ' Argentina ',
                                  ' Australia ', ' Austria ', ' Bahamas ', ' Bahrain ',
                                  ' Bangladesh ', ' Armenia ', ' Barbados ', ' Belgium ',
                                  ' Bermuda ', ' Bhutan ', ' Bolivia (Plurinational State of) ',
...
class ConfidenceDialog(QDialog):
    def __init__(self):
        ...
        # Add code for your confidence dialog initialization here
    # Add code for your confidence interval dialog initialization here
        self.f_country()
    ...
    def calculate_confidence_interval(self):
        ...
        if selected_item == 'Population Prediction Confidence Interval':
            ...
            # Calculate confidence interval for the slope (population prediction)
        ...
        # Extracting the 'Year', 'Population', and 'GDP' columns
            years = country_data['Year'].values.reshape(-1, 1)
            population = country_data['Population'].values.reshape(-1, 1)
            gdp = country_data['GDP'].values.reshape(-1, 1)

            # Create linear regression models for population and GDP
            ...
            # Calculate confidence intervals for population and GDP slopes
    ...
    def f_country(self):
        self.c.addItems(['Countries', ' Afghanistan ', ' Albania ', ' Algeria ', ' Andorra ', ' Angola ',
                                  ' Antigua and Barbuda ', ' Azerbaijan ', ' Argentina ',
                                  ' Australia ', ' Austria ', ' Bahamas ', ' Bahrain ',
                                  ' Bangladesh ', ' Armenia ', ' Barbados ', ' Belgium ',
                                  ' Bermuda ', ' Bhutan ', ' Bolivia (Plurinational State of) ',
